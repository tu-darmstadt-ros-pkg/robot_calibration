...
#include <robot_calibration/ceres/camera2d_to_arm_error.h>
  ...
  const std::string& sensor)
{
  ...
  ROS_WARN_STREAM("Data has no observation of sensor: " << sensor);
  return false;
}
                        ...
                        bool progress_to_stdout)
{
  ...
  for (size_t i = 0; i < params.models.size(); ++i)
  {
    ...
    {
      ROS_ERROR_STREAM("Unknown model type: " << params.models[i].type);
      // ERROR unknown
    }
  }
  ...
  for (size_t i = 0; i < data.size(); ++i)
  {
    ...
    for (size_t j = 0; j < params.error_blocks.size(); ++j)
    {
      ...
      else if (params.error_blocks[j].type == "camera2d_to_arm")
      {
        ROS_INFO_STREAM("Creating error block: " << params.error_blocks[j].type);
        // This error block can process data generated by the LedFinder,
        // CheckboardFinder, or any other finder that can sample the pose
        // of one or more data points that are connected at a constant offset
        // from a link a kinematic chain (the "arm").
        ...
        // Create the block
        ceres::CostFunction * cost = Camera2dToArmError::Create(
        ...
        if (progress_to_stdout)
        {
          double ** params = new double*[1];
          params[0] = free_params;
          double * residuals = new double[data[i].observations[index].features.size() * 2];  // TODO: should check that all features are same length?
      ...
      {
        ROS_ERROR_STREAM("Unknown error block type:" << params.error_blocks[j].type);
        // ERROR unknown
      }
    }
  }

  ROS_INFO_STREAM("Set up optimization.");
